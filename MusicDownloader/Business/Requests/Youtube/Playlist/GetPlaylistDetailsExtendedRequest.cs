...
public class GetPlaylistDetailsExtendedRequest : IRequest<PlaylistDetailsExtended>
{
...
public class
    GetPlaylistDetailsExtendedRequestHandler : IRequestHandler<GetPlaylistDetailsExtendedRequest,
        PlaylistDetailsExtended>
{
    ...
    public async Task<PlaylistDetailsExtended> Handle(GetPlaylistDetailsExtendedRequest metadataRequest,
        CancellationToken cancellationToken)
    {
        // Get playlist and videos
        var playlistTask = _youtube.Playlists.GetAsync(metadataRequest.Url, cancellationToken).AsTask();
        ...
        if (!metadataRequest.IncludeLiveStreams) playlistVideos = playlistVideos.Where(video => !video.IsLive).ToList();
        ...
        var tracks = playlistVideos.Select(video => new TrackDetails
        {
            ...
            Thumbnails = video.Thumbnails
                .Select(thumbnail => new ThumbnailDetails
                {
                    Url = thumbnail.Url,
