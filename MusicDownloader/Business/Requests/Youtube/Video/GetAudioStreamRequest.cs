...
public record GetAudioStreamRequest(string Url) : IRequest<DownloadStreamInfo>;

public class GetAudioStreamRequestHandler(YoutubeClient youtubeClient)
    : IRequestHandler<GetAudioStreamRequest, DownloadStreamInfo>
{
    public async Task<DownloadStreamInfo> Handle(
